...
class AsterClient:
    ...
    def __init__(
        ...
        self.lock_identifier = lock_identifier or address
        self.account_reference = account_reference  # HIBACHI-CHANGE: keep Hibachi account id on client
        self.wallet_address = wallet_address  # HIBACHI-CHANGE: keep Hibachi wallet on client
        self.label = label
        self.db = db
        if group_data:
        ...
        if self.account_reference:
            logger.opt(colors=True).debug(
                f"[•] {self.label} | HIBACHI-CHANGE account <white>{self.account_reference}</white>"
            )
        if self.wallet_address:
            logger.opt(colors=True).debug(
                f"[•] {self.label} | HIBACHI-CHANGE wallet <white>{self.wallet_address}</white>"
            )
